...
float4 GetPRTDiffuse(int iClusterOffset, float4 vPCAWeights[NUM_PCA / 4])
{
    ...
    for (int j = 0; j < (NUM_PCA / 4); j++) {
        for (int s = 0; s < 4; s++) {
            ...
            for (int k = 0; k < (NUM_COEFFS / 4); k++) {
                ...
                BRDFR[k] += vPCAWeights[j][s] * aPRTClusterBases[iClusterOffset + iPCAOffset + 0 * (NUM_COEFFS / 4) + k];
                BRDFG[k] += vPCAWeights[j][s] * aPRTClusterBases[iClusterOffset + iPCAOffset + 1 * (NUM_COEFFS / 4) + k];
                BRDFB[k] += vPCAWeights[j][s] * aPRTClusterBases[iClusterOffset + iPCAOffset + 2 * (NUM_COEFFS / 4) + k];
            }
        }
    }
    ...
    for (int t = 0; t < (NUM_COEFFS / 4); t++) {
        vDiffuse.r += dot(BRDFR[t], 1);
        vDiffuse.g += dot(BRDFG[t], 1);
        vDiffuse.b += dot(BRDFB[t], 1);
    }
